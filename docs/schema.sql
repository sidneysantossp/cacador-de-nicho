...
create table if not exists public.radar_production_quality_chapters(
  ...
  analysis_seconds numeric check(analysis_seconds is null or analysis_seconds>=0),
  error text,
  created_at timestamptz not null default now(),
  completed_at timestamptz,
  updated_at timestamptz not null default now(),
...
create index if not exists radar_production_quality_chapters_job_sequence
  on public.radar_production_quality_chapters(render_job_id,sequence);
