  ...
  return <div className="stock-media-engine">
    ...
    <section className="stock-search-controls">
      <div className="stock-search-row">
        ...
        <label><span>PROVIDER</span><select value={provider} onChange={e=>{const next=e.target.value as StockMediaProvider;setProvider(next);if(next==='unsplash'||next==='wikimedia')setKind('image');setResults([]);}}><option value="pexels">Pexels</option><option value="pixabay">Pixabay</option><option value="unsplash">Unsplash</option><option value="vecteezy">Vecteezy</option><option value="wikimedia">Wikimedia Commons</option></select></label>
        <label><span>TIPO</span><select value={kind} onChange={e=>{setKind(e.target.value as 'image'|'video');setResults([]);}}><option value="image">Imagem</option><option value="video" disabled={provider==='unsplash'||provider==='wikimedia'}>Vídeo</option></select></label>
      </div>
    ...
    {results.length>0&&<section className="stock-results">
      <div className="stock-results-head"><div><strong>{results.length} resultados</strong><small>{provider==='pexels'?'Media provided by Pexels':provider==='pixabay'?'Media provided by Pixabay':provider==='vecteezy'?'Photos and videos provided by Vecteezy':provider==='wikimedia'?'Reusable documentary images from Wikimedia Commons':'Photos provided by Unsplash'}</small></div>{rate?.remaining&&<span>API remaining: {rate.remaining}</span>}</div>
      ...
      <div className="stock-attribution"><a href={provider==='pexels'?'https://www.pexels.com':provider==='pixabay'?'https://pixabay.com':provider==='vecteezy'?'https://www.vecteezy.com':provider==='wikimedia'?'https://commons.wikimedia.org':'https://unsplash.com'} target="_blank" rel="noreferrer">{provider==='pexels'?'Photos and videos provided by Pexels':provider==='pixabay'?'Images and videos provided by Pixabay':provider==='vecteezy'?'Photos and videos provided by Vecteezy':provider==='wikimedia'?'Reusable media from Wikimedia Commons':'Photos provided by Unsplash'} <ExternalLink size={12}/></a>{provider==='unsplash'&&<small> Nesta fase o Unsplash funciona como descoberta visual com hotlink e atribuição; cópia para o Asset Vault permanece bloqueada.</small>}{provider==='vecteezy'&&<small> A busca prioriza conteúdo não gerado por IA; licença e atribuição são registradas no download.</small>}{provider==='wikimedia'&&<small> Apenas arquivos com licença reutilizável aceita pelo Source Router são exibidos; autoria, origem e licença ficam registradas.</small>}</div>
    </section>}
