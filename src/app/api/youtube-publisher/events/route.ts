...
import { listYouTubePublishJobs } from '@/lib/server/youtube-publisher';
    ...
    const stream=new ReadableStream<Uint8Array>({
      start(controller){
        ...
        void (async()=>{
          ...
          while(!stopped){
            const jobs=await listYouTubePublishJobs(channelId);
            const signature=JSON.stringify(jobs.map(job=>[
              job.id,job.status,job.progress,job.stage,job.attempts,
              job.youtubeVideoId??'',job.actualPrivacyStatus??'',job.error??'',job.updatedAt
            ]));
