...
export async function runAiJob(kind:'analyze'|'script',item:string,work:()=>Promise<unknown>){
 ...
 const run={id:key,type:kind==='analyze'?'Análise editorial':'Rascunho de roteiro',startedAt:new Date().toISOString(),status:'running',message:'Pesquisa e agentes em execução.'};
