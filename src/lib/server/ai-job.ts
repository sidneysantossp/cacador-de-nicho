...
export async function runAiJob<T>(kind:'analyze'|'script'|'channel-study'|'opportunity-report'|'universe-intelligence'|'universe-curves'|'universe-queue',item:string,work:()=>Promise<T>):Promise<T>{
 ...
 try{await put('radar_runs',key,run);const result=await work();await put('radar_runs',key,{...run,status:'completed',message:'Entrega registrada. Consulte as fontes e limitações.'});checked(await db().rpc('finish_radar_job',{job_key:key,lease_token:token,success:true}));return result;}
