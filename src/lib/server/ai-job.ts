...
export async function runAiJob(kind:'analyze'|'script'|'channel-study'|'opportunity-report'|'universe-intelligence',item:string,work:()=>Promise<unknown>){
 ...
 const run={id:key,type:kind==='analyze'?'Análise editorial':kind==='channel-study'?'Análise profunda de canal':kind==='opportunity-report'?'Opportunity Report':kind==='universe-intelligence'?'Universe Intelligence':'Rascunho de roteiro',startedAt:new Date().toISOString(),status:'running',message:'Pesquisa e agentes em execução.'};
