...
export async function settings():Promise<Settings>{return (await list<Settings>('radar_settings',1))[0]??defaultSettings;}
...
export async function loadRadar():Promise<Pick<RadarData,'channels'|'decisions'|'contexts'|'scripts'|'runs'|'settings'|'lastUpdated'>>{await cleanup();const [channels,decisions,contexts,scripts,runs,config]=await Promise.all([list<RadarData['channels'][number]>('radar_channels'),list<RadarData['decisions'][number]>('radar_decisions'),list<RadarData['contexts'][number]>('radar_contexts'),list<RadarData['scripts'][number]>('radar_scripts'),list<RadarData['runs'][number]>('radar_runs',30),settings()]);return {channels,decisions,contexts,scripts,runs,settings:config,lastUpdated:channels[0]?.observedAt??null};}
