...
import type { RadarData, Channel, ChannelBrain, ChannelBrainPayload, ChannelStudy, ManagedChannel, MissionBrief, OpportunityReport, Settings, UniverseCompetitor, UniverseMarketIntelligence } from '@/lib/types';
...
async function optionalChannelBrains():Promise<ChannelBrain[]>{
 try{
  const rows=checked(await db().from('radar_channel_brains').select('id,version,payload').order('updated_at',{ascending:false}).limit(300));
  return (rows??[]).map(row=>({...row.payload as ChannelBrainPayload,channelId:String(row.id),version:Number(row.version)}));
 }catch{return [];}
}
