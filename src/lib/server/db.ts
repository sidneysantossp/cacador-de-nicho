...
import type { RadarData, Channel, ChannelStudy, ManagedChannel, MissionBrief, OpportunityReport, Settings, UniverseCompetitor, UniverseMarketIntelligence } from '@/lib/types';
...
export async function loadRadar():Promise<Pick<RadarData,'channels'|'universeCompetitors'|'universeMarketIntelligence'|'channelStudies'|'opportunityReports'|'missionBrief'|'gaps'|'managedChannels'|'decisions'|'contexts'|'scripts'|'runs'|'settings'|'lastUpdated'>>{await cleanup();const [allChannels,allAnalyses,managedRecords,decisions,contexts,scripts,runs,config]=await Promise.all([list<RadarData['channels'][number]>('radar_channels',1000),optionalList<unknown>('radar_analyses',200),optionalList<ManagedChannel|UniverseCompetitor>('radar_managed_channels',1000),list<RadarData['decisions'][number]>('radar_decisions'),list<RadarData['contexts'][number]>('radar_contexts'),list<RadarData['scripts'][number]>('radar_scripts'),list<RadarData['runs'][number]>('radar_runs',30),settings()]);const channelStudies=allAnalyses.filter((item):item is ChannelStudy=>!!item&&typeof item==='object'&&(item as {kind?:string}).kind==='channel-study').map(normalizeChannelStudy);const opportunityReports=allAnalyses.filter((item):item is OpportunityReport=>!!item&&typeof item==='object'&&(item as {kind?:string}).kind==='opportunity-report');const universeMarketIntelligence=allAnalyses.find((item):item is UniverseMarketIntelligence=>!!item&&typeof item==='object'&&(item as {kind?:string}).kind==='universe-market-intelligence')??null;const missionBrief=allAnalyses.find((item):item is MissionBrief=>!!item&&typeof item==='object'&&(item as {kind?:string}).kind==='mission-brief')??null;const universeCompetitors=managedRecords.filter((item):item is UniverseCompetitor=>!!item&&typeof item==='object'&&(item as {kind?:string}).kind==='competitor');const managedChannels=managedRecords.filter((item):item is ManagedChannel=>!('kind' in (item as object))||(item as {kind?:string}).kind!=='competitor');const references=allChannels.filter(c=>c.discoverySource==='reference');const channels=allChannels.filter(c=>c.discoverySource==='reference-adjacent'&&qualifiesOpportunityCandidate(c,config));const gaps=buildOpportunityGaps([...references,...channels]);return {channels,universeCompetitors,universeMarketIntelligence,channelStudies,opportunityReports,missionBrief,gaps,managedChannels,decisions,contexts,scripts,runs,settings:config,lastUpdated:channels[0]?.observedAt??references[0]?.observedAt??null};}
