...
function sameUniverseOpportunityFamily(a:UniverseGap,b:UniverseGap){
  ...
  const sharedTargetTokens=[...aTokens].filter(token=>bTokens.has(token)).length;
  return sharedTargetTokens>=1;
}
...
export function selectUniverseMissionOpportunities(
  ...
  for(const candidate of ranked){
    ...
    const alternateAngles=[...(primary.alternateAngles??[]),alternate];
